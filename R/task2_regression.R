...
evaluate_regression_models <-
  ...
  cat(
  "The rate model estimates extrication rates per collision, providing insights into:\n")
  cat(
  "- Which demographic groups have higher extrication rates when crashes occur\n")
  ...
  print(
  irr_results[, c(
  "Coefficient", "IRR", "Lower_CI", "Upper_CI", "p_value")])
  cat("\n")
  ...
  if (length(age_effects) > 0) {
    ...
    for (i in seq_along(age_effects)) {
      ...
      if (age_irr > 1) {
        cat(
  "-", age_name, ": ", round(
  (age_irr - 1) * 100, 1), "% higher rate\n")
      } else {
        cat(
  "-", age_name, ": ", round(
  (1 - age_irr) * 100, 1), "% lower rate\n")
      }
    }
  }
  ...
  if (length(interaction_effects) > 0) {
    ...
    for (i in seq_along(interaction_effects)) {
      int_name <- names(interaction_effects)[i]
      int_irr <- interaction_effects[i]
      if (int_irr > 1) {
        cat(
  "-", int_name, ": Additional", round(
  (int_irr - 1) * 100, 1), "% increase for males\n")
      } else {
        cat(
  "-", int_name, ": Additional", round(
  (1 - int_irr) * 100, 1), "% decrease for males\n")
      }
    }
  }
  cat("\n")
  ...
  cat(
  "- Interpretation: Model explains", round(
  pseudo_r2 * 100, 1), "% of deviance\n\n")
...
create_regression_plots <- function(results, data) {
  ...
  interaction_data <- data %>%
    ...
    summarise(
      mean_extrication_rate = mean(
  extrication_count / collision_count, na.rm = TRUE),
      se_rate = sd(
  extrication_count / collision_count, na.rm = TRUE) / sqrt(
  n(
  )),
