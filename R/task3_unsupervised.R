...
evaluate_unsupervised_models <-
  ...
  if (max(silhouette_avg_kmeans, silhouette_avg_hclust) > 0.5) {
    quality_interpretation <-
   "- INTERPRETATION: Strong cluster structure - clear olive oil types identified\n"
  } else if (max(silhouette_avg_kmeans, silhouette_avg_hclust) > 0.25) {
    quality_interpretation <-
   "- INTERPRETATION: Reasonable cluster structure - distinct olive oil groups present\n"
  } else {
    quality_interpretation <-
   "- INTERPRETATION: Weak cluster structure - overlapping olive oil characteristics\n"
  }
...
create_unsupervised_plots <- function(results, pca_result, data) {
  ...
  if (!is.null(results$kmeans_clusters)) {
   ...
   silhouette(results$kmeans_clusters, dist(data$scaled_data))
    ...
    plots$silhouette <-
   ggplot(silhouette_df, aes(x = Sample, y = Silhouette_Width, fill = Cluster)) +
